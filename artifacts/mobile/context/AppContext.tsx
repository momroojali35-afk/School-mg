...
export const STUDENT_CASTE_OPTIONS = [
  "General",
  "OBC",
  "SC",
  "ST",
 ...
 */
export function compareStudentRollNumbers(
  a: Pick<Student, "rollNumber" | "name">,
  b: Pick<Student, "rollNumber" | "name">,
): number {
  ...
  const rollOrder = aRoll.localeCompare(bRoll, undefined, {
    numeric: true,
    sensitivity: "base",
  });
  return (
...
export function getStudentFeeInfo(student: Student, feeRecords: FeeRecord[]) {
  ...
  const annualRecords = feeRecords.filter(
    (f) =>
      f.studentId === student.id &&
      (f.feeCategory === "annual" ||
        (!f.feeCategory &&
          f.feeTypeName?.toLowerCase().includes("annual fee"))),
  );
 ...
 */
export function getExamSubjectsForClass(
  exam: Exam,
  className: string,
): string[] {
 ...
 */
export function getExamScheduleForClass(
  exam: Exam,
  className: string,
  ...
  if (exam.classSubjects && exam.classSubjects.length > 0) {
    ...
    if (entry)
      return (
        entry.subjectSchedule ?? entry.subjectSchedules ?? exam.subjectSchedule
      );
  }
...
export function getSubjectMaxMarksForClass(
  ...
  const schedule = getExamScheduleForClass(exam, className ?? exam.class)?.find(
    (item) => item.subject === subject,
  );
  const scheduledMax = Number(
    schedule?.maxMarks ??
      (schedule as (SubjectSchedule & { max_marks?: number }) | undefined)
        ?.max_marks,
  );
...
export function compareSalaryRecordsNewestFirst(
  a: SalaryRecord,
  b: SalaryRecord,
): number {
  const periodOrder =
...
const SEED_FEE_RECORDS: FeeRecord[] = [
  {
    ...
    amount: 3500,
    date: todayStr,
    description: "Monthly Fee - July 2026",
    feeTypeId: "ft1",
...
const SEED_RESULTS: ExamResult[] = [
  {
    ...
    class: "Class 5",
    rollNumber: "01",
    marks: { Mathematics: 85, Science: 78, English: 90, Hindi: 82 },
  },
  {
    id: "r2",
    examId: "e1",
    ...
    class: "Class 5",
    rollNumber: "02",
    marks: { Mathematics: 92, Science: 88, English: 95, Hindi: 89 },
  },
];
...
const genReceiptNumber = (prefix = "RCP") =>
  `${prefix}${String(receiptCounter++).padStart(8, "0")}`;
  ...
  return {
    ...
    classSubjects: Array.isArray(cs)
      ? cs.map((assignment: any) => ({
          ...assignment,
          subjectSchedule:
            assignment.subjectSchedule ?? assignment.subjectSchedules,
        }))
      : undefined,
    date: r.date,
    maxMarks: r.maxMarks ?? r.max_marks ?? 100,
  };
}
...
function mapFeeRecord(r: any): FeeRecord {
  return {
    ...
    date: r.date,
    description: r.description ?? "",
    feeTypeId: r.feeTypeId ?? r.fee_type_id ?? undefined,
    feeTypeName: r.feeTypeName ?? r.fee_type_name ?? undefined,
    collectedBy: r.collectedBy ?? r.collected_by ?? "",
    receiptNumber: r.receiptNumber ?? r.receipt_number ?? undefined,
...
function mapSalary(r: any): SalaryRecord {
  return {
    ...
    amount: r.amount,
    status: r.status,
    paidDate: r.paidDate ?? r.paid_date ?? undefined,
    receiptNumber: r.receiptNumber ?? r.receipt_number ?? undefined,
  };
}
...
function mapPromotion(r: any): PromotionRecord {
  return {
    ...
    promotedBy: r.promotedBy ?? r.promoted_by ?? "",
    promotedAt: r.promotedAt ?? r.promoted_at ?? "",
  };
}
...
async function seedAllData() {
  try {
    await Promise.allSettled(
      SEED_CLASSES.map((name) => apiPost("/classes", { name })),
    );
    await Promise.allSettled(
      SEED_SUBJECTS.map((name) => apiPost("/subjects", { name })),
    );
    ...
    await Promise.allSettled(
      SEED_FEE_TYPES.map((f) => apiPost("/fee-types", f)),
    );
    await Promise.allSettled(SEED_EXAMS.map((e) => apiPost("/exams", e)));
    await Promise.allSettled(
      SEED_FEE_RECORDS.map((f) => apiPost("/fee-records", f)),
    );
    await Promise.allSettled(SEED_EXPENSES.map((e) => apiPost("/expenses", e)));
    await Promise.allSettled(
      SEED_SALARIES.map((s) => apiPost("/salary-records", s)),
    );
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadAllData = React.useCallback(async (): Promise<boolean> => {
    try {
      const [
        ...
        apiGet<any[]>("/inactivation-requests", { cache: "no-store" }).catch(
          () => [] as any[],
        ),
        apiGet<Record<string, number>>("/settings/class-absent-limits").catch(
          () => ({}) as Record<string, number>,
        ),
        apiGet<DocumentBranding>("/settings/document-branding").catch(
          () => DEFAULT_STATE.documentBranding,
        ),
        apiGet<any[]>("/alumni").catch(() => [] as any[]),
      ]);

      const isEmpty =
        classes.length === 0 && students.length === 0 && teachers.length === 0;
      ...
      if (isEmpty) {
        ...
        setState({
          classes: c2.sort((a: string, b: string) =>
          ...
          sections: sec2,
          students: s2.map(mapStudent),
          teachers: t2.map(mapTeacher),
          subjects: sub2,
          feeTypes: ft2.map(mapFeeType),
          attendanceRecords: att2.map(mapAttendance),
          exams: ex2.map(mapExam),
          examResults: res2.map(mapExamResult),
          feeRecords: fee2.map(mapFeeRecord),
          expenses: exp2.map(mapExpense),
          salaryRecords: sal2.map(mapSalary),
          promotionRecords: pro2.map(mapPromotion),
          markSubmissions: ms2,
          markAuditLog: auditLog2,
        ...
        setState({
          classes: classes.sort((a: string, b: string) =>
            a.localeCompare(b, undefined, {
              numeric: true,
              sensitivity: "base",
            }),
          ...
          markSubmissions: markSubs,
          markAuditLog: auditLog,
  ...
  const syncPendingDbConfig = useCallback(async () => {
    try {
      ...
      const result = await apiPost<{ success: boolean }>(
        `/db-connections/${created.id}/test`,
        {},
      );
      if (result.success) {
        ...
        await fetch(`${getApiBase()}/api/db-connections/${created.id}`, {
          method: "DELETE",
        }).catch(() => {});
      }
    } catch (_e) {
      ...
      .then((row) => {
        setState((prev) => ({
          ...
          students: prev.students.map((x) =>
            x.id === ns.id ? mapStudent(row as any) : x,
          ),
        }));
      })
      .catch(console.error);
      ...
      .then((row) => {
        setState((prev) => ({
          ...
          teachers: prev.teachers.map((x) =>
            x.id === nt.id ? mapTeacher(row as any) : x,
          ),
        }));
      })
      .catch(console.error);
  ...
  const updateTeacher = useCallback((id: string, t: Partial<Teacher>) => {
    setState((prev) => ({
      ...prev,
      teachers: prev.teachers.map((x) => (x.id === id ? { ...x, ...t } : x)),
    }));
    apiPut(`/teachers/${id}`, t)
      .then((row) => {
        setState((prev) => ({
          ...prev,
          teachers: prev.teachers.map((x) =>
            x.id === id ? mapTeacher(row as any) : x,
          ),
        }));
      })
      .catch(console.error);
  ...
  const addClass = async (name: string): Promise<void> => {
    ...
    setState((prev) => {
      ...
      return {
        ...prev,
        classes: [...prev.classes, name].sort((a, b) =>
  ...
  const updateClass = async (
    ...
    const res = await fetch(
      `${getApiBase()}/api/classes/${encodeURIComponent(oldName)}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const deleteClass = async (name: string): Promise<void> => {
    const res = await fetch(
      `${getApiBase()}/api/classes/${encodeURIComponent(name)}`,
      { method: "DELETE" },
    );
    if (res.status === 404) throw new Error("Class not found");
  ...
  const updateSection = async (
    ...
    const trimmed = newName.trim();
    const res = await fetch(
      `${getApiBase()}/api/sections/${encodeURIComponent(oldName)}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ newName: trimmed }),
    ...
    setState((prev) => ({
      ...prev,
      sections: Array.from(
        new Set(prev.sections.map((s) => (s === oldName ? trimmed : s))),
      ).sort(),
      students: prev.students.map((student) =>
        student.section === oldName
          ? { ...student, section: trimmed }
          : student,
      ),
    }));
  };
  ...
  const deleteSection = async (name: string): Promise<void> => {
    const res = await fetch(
      `${getApiBase()}/api/sections/${encodeURIComponent(name)}`,
      { method: "DELETE" },
    );
    if (res.status === 404) throw new Error("Section not found");
    ...
    setState((prev) => ({
      ...
      students: prev.students.map((student) =>
        student.section === name ? { ...student, section: undefined } : student,
      ),
    }));
  };
  ...
  const addFeeType = useCallback((f: Omit<FeeType, "id">) => {
    ...
    apiPost("/fee-types", nf)
      .then((row) => {
        setState((prev) => ({
          ...prev,
          feeTypes: prev.feeTypes.map((x) =>
            x.id === nf.id ? mapFeeType(row as any) : x,
          ),
        }));
      })
      .catch(console.error);
  ...
  const updateFeeType = useCallback((id: string, f: Partial<FeeType>) => {
    setState((prev) => ({
      ...prev,
      feeTypes: prev.feeTypes.map((x) => (x.id === id ? { ...x, ...f } : x)),
    }));
  ...
  const deleteFeeType = useCallback((id: string) => {
    setState((prev) => ({
      ...prev,
      feeTypes: prev.feeTypes.filter((x) => x.id !== id),
    }));
    ...
    (records: Omit<AttendanceRecord, "id">[]) => {
      ...
      setState((prev) => {
        const filtered = prev.attendanceRecords.filter(
          (r) =>
            !(r.date === records[0]?.date && r.class === records[0]?.class),
        );
        return { ...prev, attendanceRecords: [...filtered, ...nr] };
      });
      apiPost<any>("/attendance", nr)
        .then((resp) => {
          ...
          if (resp?.records && Array.isArray(resp.records)) {
            setState((prev) => {
              ...
              return {
                ...
                attendanceRecords: [
                  ...filtered,
                  ...resp.records.map(mapAttendance),
                ],
              };
            });
          }
          ...
          if (
            resp?.inactivated &&
            Array.isArray(resp.inactivated) &&
            resp.inactivated.length > 0
          ) {
            setState((prev) => ({
              ...prev,
              students: prev.students.map((s) =>
                resp.inactivated.includes(s.id)
      ...
      .then((row) => {
        setState((prev) => ({
          ...
          exams: prev.exams.map((x) =>
            x.id === ne.id ? mapExam(row as any) : x,
          ),
        }));
      })
      .catch(console.error);
  ...
  const updateExam = useCallback((id: string, e: Partial<Exam>) => {
    setState((prev) => ({
      ...prev,
      exams: prev.exams.map((x) => (x.id === id ? { ...x, ...e } : x)),
    }));
  ...
  // ── Mark Submissions ──
  const submitSubjectMarks = useCallback(
    async (params: {
      examId: string;
      class: string;
      subject: string;
      ...
      setState((prev) => {
        ...
        const existingStudentIds = new Set(
          prev.examResults
            .filter((r) => r.examId === examId && r.class === cls)
            .map((r) => r.studentId),
        );
            ...
            ? [
                {
                  ...
                  marks: { [subject]: m.mark },
                },
              ]
            : [],
        );
        const subExists = prev.markSubmissions.some(
          (s) =>
            s.examId === examId && s.class === cls && s.subject === subject,
        );
        const newMarkSubs = subExists
          ? prev.markSubmissions.map((s) =>
              s.examId === examId && s.class === cls && s.subject === subject
                ? submission
                : s,
            )
        ...
        return {
          ...prev,
          examResults: [...updated, ...newRows],
          markSubmissions: newMarkSubs,
        };
      });
    },
    [],
  ...
  const unlockSubject = useCallback(
    async (
      examId: string,
      cls: string,
      subject: string,
  ...
  const saveExamResults = useCallback((results: Omit<ExamResult, "id">[]) => {
    ...
    setState((prev) => {
      ...
      const filtered = examId
        ? prev.examResults.filter(
            (r) => !(r.examId === examId && r.class === classFilter),
          )
        : prev.examResults;
  ...
  const addFeeRecord = useCallback((r: Omit<FeeRecord, "id">): FeeRecord => {
    const nr: FeeRecord = {
      ...r,
      id: genId(),
      receiptNumber: r.receiptNumber ?? genReceiptNumber(),
    };
    setState((prev) => ({ ...prev, feeRecords: [...prev.feeRecords, nr] }));
    apiPost("/fee-records", nr)
      .then((row) => {
        setState((prev) => ({
          ...prev,
          feeRecords: prev.feeRecords.map((x) =>
            x.id === nr.id ? mapFeeRecord(row as any) : x,
          ),
        }));
      })
      .catch(console.error);
  ...
  const deleteFeeRecord = useCallback((id: string) => {
    setState((prev) => ({
      ...prev,
      feeRecords: prev.feeRecords.filter((x) => x.id !== id),
    }));
      ...
      .then((row) => {
        setState((prev) => ({
          ...
          expenses: prev.expenses.map((x) =>
            x.id === ne.id ? mapExpense(row as any) : x,
          ),
        }));
      })
      .catch(console.error);
  ...
  const updateSalaryStatus = useCallback(
    (
      ...
      setState((prev) => {
        const existing = prev.salaryRecords.find(
          (r) =>
            r.teacherId === teacherId && r.month === month && r.year === year,
        );
        if (existing) {
          updated = {
            ...existing,
            status,
            paidDate: status === "paid" ? todayStr : undefined,
          };
          const records = prev.salaryRecords.map((r) =>
            r.id === existing.id ? updated! : r,
          );
          apiPut(`/salary-records/${existing.id}`, updated).catch(
            console.error,
          );
          ...
          updated = {
            ...
            status,
            paidDate: status === "paid" ? todayStr : undefined,
            receiptNumber: genReceiptNumber("SAL"),
          };
  ...
  const addSalaryRecord = useCallback(
    (r: Omit<SalaryRecord, "id">): SalaryRecord => {
      const nr: SalaryRecord = {
        ...r,
        id: genId(),
        receiptNumber: r.receiptNumber ?? genReceiptNumber("SAL"),
      };
      setState((prev) => ({
        ...prev,
        salaryRecords: [...prev.salaryRecords, nr],
      }));
  ...
  const deleteSalaryRecord = useCallback((id: string) => {
    setState((prev) => ({
      ...prev,
      salaryRecords: prev.salaryRecords.filter((x) => x.id !== id),
    }));
  ...
  const promoteStudent = useCallback(
    (studentId: string, toClass: string, promotedBy: string) => {
      ...
      setState((prev) => {
        ...
        return {
          ...prev,
          students: prev.students.map((s) =>
            s.id === studentId ? { ...s, class: toClass } : s,
          ),
  ...
  const bulkPromoteClass = useCallback(
    (fromClass: string, toClass: string, promotedBy: string): number => {
      let count = 0;
      setState((prev) => {
        const toPromote = prev.students.filter(
          (s) => s.class === fromClass && isActiveStudent(s),
        );
        count = toPromote.length;
        if (count === 0) return prev;
        const records: PromotionRecord[] = toPromote.map((s) => ({
          id: genId(),
          studentId: s.id,
          studentName: s.name,
          fromClass,
        ...
        return {
          ...prev,
          students: prev.students.map((s) =>
            s.class === fromClass && isActiveStudent(s)
              ? { ...s, class: toClass }
              : s,
          ),
  ...
  // ── Inactivation Requests ──
  const submitInactivationRequest = useCallback(
    async (data: {
      studentId: string;
      studentName: string;
      studentClass: string;
      ...
      setState((prev) => ({
        ...prev,
        inactivationRequests: [req, ...prev.inactivationRequests],
      }));
      return req;
    },
    [],
  ...
  const approveInactivationRequest = useCallback(
    async (id: string, adminNote?: string): Promise<void> => {
      const row = await apiPut<any>(`/inactivation-requests/${id}/approve`, {
        adminNote,
      });
      const req = mapInactivationRequest(row);
      setState((prev) => ({
        ...prev,
        inactivationRequests: prev.inactivationRequests.map((r) =>
          r.id === id ? req : r,
        ),
        // Reactivate student in local state
        students: prev.students.map((s) =>
          s.id === req.studentId ? { ...s, status: "active" as const } : s,
        ),
      }));
    },
    [],
  ...
  const rejectInactivationRequest = useCallback(
    async (id: string, adminNote?: string): Promise<void> => {
      const row = await apiPut<any>(`/inactivation-requests/${id}/reject`, {
        adminNote,
      });
      const req = mapInactivationRequest(row);
      setState((prev) => ({
  ...
  const deleteInactivationRequestDocument = useCallback(
  ...
  const deleteInactivationRequest = useCallback(
    async (id: string): Promise<void> => {
      const result = await apiDelete<{ deleted?: boolean; id?: string }>(
        `/inactivation-requests/${id}`,
      );
      if (!result?.deleted || result.id !== id) {
        throw new Error(
          "The request was not confirmed as deleted from the database.",
        );
      }
      ...
      const rows = await apiGet<any[]>(
        `/inactivation-requests?refresh=${Date.now()}`,
        { cache: "no-store" },
      );
      if (rows.some((row) => row.id === id)) {
        throw new Error(
          "The request still exists in the database. It was not deleted.",
        );
      }
      setState((prev) => ({
        ...prev,
        inactivationRequests: rows.map(mapInactivationRequest),
      }));
    },
    [],
  );
  ...
  const setClassAbsentLimit = useCallback(
    async (className: string, maxDays: number): Promise<void> => {
      const limits = await apiPut<Record<string, number>>(
        "/settings/class-absent-limits",
        {
          className,
          maxDays,
          adminId: "admin",
  ...
  const updateDocumentBranding = useCallback(
    async (branding: DocumentBranding): Promise<void> => {
      const saved = await apiPut<DocumentBranding>(
  ...
  const addAlumni = useCallback((a: Omit<Alumni, "id">) => {
    ...
    apiPost("/alumni", na)
      .then((row) => {
        setState((prev) => ({
          ...prev,
          alumni: prev.alumni.map((x) =>
            x.id === na.id ? (row as Alumni) : x,
          ),
        }));
      })
      .catch(console.error);
  ...
  const updateAlumni = useCallback((id: string, a: Partial<Alumni>) => {
    setState((prev) => ({
      ...prev,
      alumni: prev.alumni.map((x) => (x.id === id ? { ...x, ...a } : x)),
    }));
  ...
  const deleteAlumni = useCallback((id: string) => {
    setState((prev) => ({
      ...prev,
      alumni: prev.alumni.filter((x) => x.id !== id),
    }));
    apiDelete(`/alumni/${id}`).catch(console.error);
  }, []);

  const bulkAddAlumni = useCallback(
    async (
      records: Omit<Alumni, "id" | "batch">[],
      batch: string,
    ): Promise<void> => {
      const withIds: Alumni[] = records.map((r) => ({
        ...r,
        id: genId(),
        batch,
      }));
      ...
      try {
        ...
        setState((prev) => ({
          ...prev,
          alumni: prev.alumni.filter(
            (x) => !withIds.some((w) => w.id === x.id),
          ),
        }));
  ...
  return (
    <AppContext.Provider
      value={{
        ...
        approveInactivationRequest,
        rejectInactivationRequest,
        deleteInactivationRequestDocument,
        deleteInactivationRequest,
        refreshInactivationRequests,
        setStudentStatus,
        setClassAbsentLimit,
