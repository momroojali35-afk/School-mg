...
**How to apply:** Any future Alumni import or migration must preserve `student_id` uniqueness, map `student_name`, `class`, `section`, and `graduation_year`, use null/default-safe optional values, and refresh from the server after writes.